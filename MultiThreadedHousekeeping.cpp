...
void* setupWordTopicLabel(void * data){
    ...
    for(int j = lda_data->start; j < lda_data->end; j++){
        for(string n : wordsInAbstracts.at(j)){
            wordTopicLabel.at(j).push_back(createWordTopics(n, i));
            totalWords++;
        }
        i++;
    }
...
void* setupDocuTopicCount(void* data){
    struct LDAData* lda_data = (struct LDAData*)data;
    for(int j = lda_data->start; j < lda_data->end; j++){
            docuTopicCount.at(j) = createDocuTopicMatrix(titlesAndAbstracts.at(j).at(0));
    }
    return 0;
}

void* setDataSet(void* data){
struct LDAData* lda_data = (struct LDAData*)data;
